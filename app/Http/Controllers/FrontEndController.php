...
//dd(auth()->user()->id);
           ...
           $response = new \Illuminate\Http\Response(view('welcome',[
               'sections' =>[['super_title'=>__('Cities'),'title'=>__('Find us in these cities and many more!'),'cities'=>Restorant::where('city_id','=',17)->get()]],

               'orders_user' =>[['orders_user'=>'orders_user','title'=>'orders_user','orders_user'=>Order::where('client_id','=',auth()->user()->id)->orderByRaw('id desc')->get()]],
               'orders' =>[['super_title'=>'Нарачки','title'=>'Активни нарачки','orders'=>DB::table('orders')->select('*', 'order_has_status.created_at as statustime')
               ->join('order_has_status', 'orders.id', '=', 'order_has_status.order_id')
               ->get()]],
...
//            'orders' =>[['super_title'=>'Нарачки','title'=>'Активни нарачки','orders'=>Order::where('client_id','=',auth()->user()->id)->get()]],
     ...
     */
    public function showStores_new($city_alias){
        //Variants
        /**
        ...
        if($hasCity){
            ...
            if($geoResults['formatted_address']=="result_not_found"){
                //No results found
                return view('restorants.error_location',['message'=>'Внесената адреса не може да биде пронајдена!']);
            }else{
            ...
            $restorants=$this->filterRestaurantsOnQuery(Restorant::xwhere(['active' => 1])->pluck('id')->toArray());
    ...
    public function showStores($city_alias){
        ...
        return $response->with('name', 'Victoria');
