        ...
        <div class="row">
            <div class="col-xl-6">
                ...
                <div class="card bg-secondary shadow">
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            <div class="col-8">
                                <h3 class="mb-0">Менаџирање на ресторан</h3>
                            ...
                            <div class="col-4 text-right">
                                @if(auth()->user()->hasRole('admin'))
                                    <a href="{{ route('restorants.index') }}" class="btn btn-sm btn-primary">Врати се назад</a>
                                     <a href="{{ route('admin.items',$restorant) }}" class="btn btn-sm btn-default">Мени</a>
                                @endif
                                @if (env('WILDCARD_DOMAIN_READY'))
                                    <a target="_blank" href="{{ (isset($_SERVER['HTTPS'])&&$_SERVER["HTTPS"] ?"https://":"http://").$restorant->subdomain.".".$_SERVER['HTTP_HOST'] }}" class="btn btn-sm btn-success">Погледни</a>
                                @else
                                    <a target="_blank" href="{{ route('vendor',$restorant->subdomain) }}" class="btn btn-sm btn-success">Погледни</a>
                                @endif
                            ...
                            <form method="post" action="{{ route('restorants.update', $restorant) }}" autocomplete="off" enctype="multipart/form-data">
                                ...
                                @method('put')
                                    ...
                                    @include('partials.fields',['fields'=>[
                                         ['ftype'=>'input','name'=>"Име на ресторан",'id'=>"name",'placeholder'=>"Име",'required'=>true,'value'=>$restorant->name],
                                         ['ftype'=>'input','name'=>"Опис за ресторанот",'id'=>"description",'placeholder'=>"Опис",'required'=>true,'value'=>$restorant->description],
                                         ['ftype'=>'input','name'=>"Адреса на ресторанот",'id'=>"address",'placeholder'=>"Адреса",'required'=>true,'value'=>$restorant->address],
                                    ]])

                                    @if (env('MULTI_CITY',false))
                                        @include('partials.fields',['fields'=>[
                                            ['ftype'=>'select','name'=>"Град во кој се наоѓа ресторанот",'id'=>"city_id",'data'=>$cities,'required'=>true,'value'=>$restorant->city_id],
                                    ...
                                    <div class="form-group{{ $errors->has('minimum') ? ' has-danger' : '' }}">
                                        <label class="form-control-label" for="input-description">Минимум сум на нарачка</label>
                                        <input type="number" name="minimum" id="input-minimum" class="form-control form-control-alternative{{ $errors->has('minimum') ? ' is-invalid' : '' }}" placeholder="{{ __('Внесете минимум сума') }}" value="{{ old('minimum', $restorant->minimum) }}" required autofocus>
                                    ...
                                    @if(auth()->user()->hasRole('admin'))
                                        ...
                                        <div class="row">
                                            <div class="col-6 form-group{{ $errors->has('fee') ? ' has-danger' : '' }}">
                                                <label class="form-control-label" for="input-description">{{ __('Процент за провизија') }}</label>
                                    ...
                                    <div class="row">
                                        <?php
                                            $images=[
                                                ['name'=>'resto_logo','label'=>__('Лого на ресторанот'),'value'=>$restorant->logom,'style'=>'width: 295px; height: 200px;'],
                                                ['name'=>'resto_cover','label'=>__('Кавер слика'),'value'=>$restorant->coverm,'style'=>'width: 200px; height: 100px;']
                                            ]
                                        ?>
                ...
                <div class="col-xl-6 mb-5 mb-xl-0">
                    ...
                    <div class="card card-profile shadow">
                        <div class="card-header">
                            <h5 class="h3 mb-0">Локација и регион на достава на ресторанот</h5>
                        </div>
                        <div class="card-body">
                            <div class="nav-wrapper">
                                <ul class="nav nav-pills nav-fill flex-column flex-md-row" id="tabs-icons-text" role="tablist">
                                    <li class="nav-item">
                                        <a class="nav-link mb-sm-3 mb-md-0 active" id="tabs-icons-text-1-tab" data-toggle="tab" href="#tabs-icons-text-1" role="tab" aria-controls="tabs-icons-text-1" aria-selected="true">Локација</a>
                                    </li>
                                    @if (!env('DISABLE_DELIVER',false))
                                        <li class="nav-item">
                                            <a class="nav-link mb-sm-3 mb-md-0" id="tabs-icons-text-2-tab" data-toggle="tab" href="#tabs-icons-text-2" role="tab" aria-controls="tabs-icons-text-2" aria-selected="false">Област за испорака</a>
                                        </li>
                                    @endif
                                </ul>
                            </div>
                                ...
                                <div class="card-body">
                                    <div class="tab-content" id="myTabContent">
                                        ...
                                        <div class="tab-pane fade" id="tabs-icons-text-2" role="tabpanel" aria-labelledby="tabs-icons-text-2-tab">
                                            ...
                                            <br/>
                                            <button type="button" id="clear_area" class="btn btn-danger btn-sm btn-block">Отстрани област за испорака</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="card card-profile bg-secondary shadow">
                    <div class="card-header">
                        <h5 class="h3 mb-0">Работно време(денови и период)</h5>
                    </div>
                    <div class="card-body">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-3">
                                            <div class="input-group">
                                                ...
                                                <input id="{{ $key.'_from' }}" name="{{ $key.'_from' }}" class="flatpickr datetimepicker form-control" type="text" placeholder="Час">
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-3">
                                            <div class="input-group">
                                                ...
                                                <input id="{{ $key.'_to' }}" name="{{ $key.'_to' }}" class="flatpickr datetimepicker form-control" type="text" placeholder="Час">
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
