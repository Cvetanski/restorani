    ...
    <form role="form" method="post" action="{{ route('rate.order', isset($order)?$order:"") }}">
        ...
        <section class='rating-widget'>
            ...
            <div id="order_comment" class="form-group{{ $errors->has('comment') ? ' has-danger' : '' }}">
                <label class="form-control-label" for="name">{{ __('Коментар за нарачката') }}</label>
                <textarea name="comment" id="comment" class="form-control{{ $errors->has('comment') ? ' is-invalid' : '' }}" placeholder="{{ __( 'Напишете го вашиот коментар тука' ) }} ..." required></textarea>
